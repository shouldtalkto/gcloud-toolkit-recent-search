...
async function recentSearch(reqBody, nextToken?) {
	...
	const rcntSearch = reqBody.recentSearch
	let query =
		config.recent_search_url +
		'&query=' +
		rcntSearch.query +
		'&max_results=' +
		rcntSearch.maxResults
	if (nextToken != undefined && nextToken != null) query = query + '&next_token=' + nextToken
