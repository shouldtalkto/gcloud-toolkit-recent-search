...
async function insertTweets(data, reqBody) {
	...
	data.forEach(function (tweet, index) {
		...
		if (tweet) {
			const cDate = new Date(tweet.created_at)
			if (tweet.context_annotations === undefined) tweet.context_annotations = []
			if (tweet.referenced_tweets === undefined) tweet.referenced_tweets = []
			let row = {
				id: tweet.id,
				text: tweet.text,
...
async function insertSearchResults(results, reqBody) {
	...
	if (data != undefined) insertTweets(data, reqBody)
	let users = results.includes.users
	if (users != undefined) insertUsers(users, reqBody)
	let media = results.includes.media
	if (media != undefined) insertMedia(media, reqBody)
}
